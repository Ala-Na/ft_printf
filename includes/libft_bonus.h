...
/*   Updated: 2021/05/19 17:36:13 by elanna           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void					ft_contadd_back(t_content **alst,
t_content *new);
void					ft_delcont(int fd, t_content **content);
t_content				*ft_contchr_fd(t_content **lst, int fd);
int						get_content_line(int fd,
char **line, ssize_t rd, t_content **cont);
void					fill_content_struct(t_content **content,
char *buffer, int fd);
...
int						exp_after_rounding(
char **number, int exp, int precision, int int_size);
void					adjust_precision(char *int_part,
char *frac_part, int *precision);
void					adjust_number(char **number);

char					*check_exp_for_format(int exp,
char *int_part, char *frac_part, int precision);
char					*get_number_shortest_rep(char *int_part,
char *frac_part, int precision);
char					*get_dtoa_shortest_rep(
unsigned long long mant, short exp, int precision);
